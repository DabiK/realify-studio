...
A recurring cast does not require every character in every frame. Select who is needed for the action. Do not repeatedly place the same trio left/center/right at the same scale. Treat a previous frame as identity and world continuity, not a layout template. In the shot plan, contrast consecutive frames through camera distance, physical viewpoint or cast selection; in the review, flag repeated blocking even if the planned prompts differed. Preserve an observed flaw for targeted feedback rather than claiming the plan proves the output's quality.

...
`job.recover` valide et importe les fichiers existants d’un job en échec, sans demander de nouvelles images. Il exige l’absence de travail actif sur le post et un `result.txt` final de Codex. Sans ce compte rendu, l’agent doit vérifier l’arrêt du processus avant une récupération manuelle ; la commande ne devine pas qu’un processus encore actif s’est arrêté. Un résultat partiel reste en échec et conserve les images valides récupérées.
